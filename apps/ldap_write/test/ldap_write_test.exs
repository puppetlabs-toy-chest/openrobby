defmodule LdapWriteWorkerTest do
  ...
  test "given an error message from LDAP, handle_write_results returns the appropriate error message" do
    assert Worker.handle_write_results({:error, {:response, :ldap_reason}}) ==
             {:error, :ldap_reason}
  ...
  defp changeset(params) do
    %{
      ...
      model: %{
        ...
        onCall: nil,
        github: nil,
        skills: ["tying knots", "breaking rocks", "matching socks"],
        id: nil,
        googleGroups: nil,
        projects: nil,
        pseudonym: "nickname",
  ...
  test "given a profile changeset, categorize_changes returns the right list of ldap modification operations" do
    ...
    assert Worker.categorize_changes(changeset) ==
             [
               {:ModifyRequest_changes_SEQOF, :add, {:PartialAttribute, 'skills', ["elixir"]}},
               {:ModifyRequest_changes_SEQOF, :add, {:PartialAttribute, 'skills', ["ruby"]}},
               {:ModifyRequest_changes_SEQOF, :delete,
                {:PartialAttribute, 'skills', ["matching socks"]}},
               {:ModifyRequest_changes_SEQOF, :delete,
                {:PartialAttribute, 'skills', ["tying knots"]}}
             ]
    ...
    assert Worker.categorize_changes(changeset2) ==
             [
               {:ModifyRequest_changes_SEQOF, :delete,
                {:PartialAttribute, 'bribeMeWith', ["skittles"]}},
               ...
               {:ModifyRequest_changes_SEQOF, :replace,
                {:PartialAttribute, 'pseudonym', ["jiff"]}}
             ]

    changeset3 =
      changeset(%{
        skills: ["tying knots", "breaking rocks", "matching socks", "trolling"],
        languages: ["Tagalog", "Malagasy", "Woods Cree"],
        bribeMeWith: "socks to match"
      })

    assert Worker.categorize_changes(changeset3) ==
             [
               {:ModifyRequest_changes_SEQOF, :replace,
                {:PartialAttribute, 'bribeMeWith', ["socks to match"]}},
               {:ModifyRequest_changes_SEQOF, :add,
                {:PartialAttribute, 'languages', ["Malagasy"]}},
               {:ModifyRequest_changes_SEQOF, :add,
                {:PartialAttribute, 'languages', ["Tagalog"]}},
               {:ModifyRequest_changes_SEQOF, :add,
                {:PartialAttribute, 'languages', ["Woods Cree"]}},
               {:ModifyRequest_changes_SEQOF, :add, {:PartialAttribute, 'skills', ["trolling"]}}
             ]
  end
end
