defmodule RobbyWeb.PasswordChangeController do
  ...
  def action(conn, _) do
    apply(__MODULE__, action_name(conn), [conn, conn.params, [conn.assigns.repo_user, conn.assigns.current_user]])
  end
  ...
  defp update_salt(conn, repo_user) do
    {salt_message, changeset} = repo_user
  ...
  def create(conn, %{"password_change" => %{ "new_password" => new_password, "old_password" => old_password, "confirm_new_password" => new_password}}, [repo_user, _current_user]) do
    PasswordPolicy.passes?(conn.assigns.max_effective_policy, new_password)
    |> case do
      {:error, reason} -> handle_response(conn, reason)
      :ok ->
        case LdapWrite.Worker.change_password_as_user(conn.assigns.ldap_user.dn, new_password, old_password) do
          {:error, reason} -> handle_response(conn, reason)
          :ok ->
  ...
  def create(conn, %{"password_change" => %{ "new_password" => _new_password, "old_password" => _old_password, "confirm_new_password" => _confirm_new_password}}, [_repo_user, _current_user]) do
    conn
