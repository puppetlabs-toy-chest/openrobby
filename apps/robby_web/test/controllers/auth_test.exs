defmodule RobbyWeb.AuthTest do
  ...
  test "login_by_email_and pass with valid LDAP creds. returns :ok and a logged in connection", meta do
    ...
    conn = with_session(build_conn(), nil)
    {resp, conn} = Auth.login_by_user_and_pass(conn, "jane", "ggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggg", repo: Repo)
    assert resp == :ok
  ...
  test "login_by_email_and_pass with invalid LDAP creds does not add a user to the session" do
    conn = with_session(build_conn(), nil)
    assert Auth.login_by_user_and_pass(conn, "jim", "g", repo: Repo) == {:error, :unauthorized, conn}
    refute Repo.get_by(User, username: "nobody")
  end

  test "logout drops user info from session" do
    conn = with_session(build_conn(), @valid_user)
  ...
  test "authenticate_user returns conn if user is logged in", %{repo_user: repo_user} do
    conn = with_session(build_conn(), repo_user.id)
          |> Auth.call(RobbyWeb.Repo)
