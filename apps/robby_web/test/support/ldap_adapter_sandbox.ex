defmodule RobbyWeb.Ldap.Adapter.Sandbox do
  ...
  def handle_call({:search, %{scope: :baseObject, base: 'uid=jim,ou=users,dc=example,dc=com'}}, _from, state) do
    ldap_response = {:ok, {:eldap_search_result, [List.first(state)], []}}
    {:reply, ldap_response, state}
  end
  def handle_call({:search, %{scope: :baseObject, base: 'uid=matt,ou=users,dc=example,dc=com'}}, _from, state) do
  ...
  def handle_call({:search, %{base: 'ou=users,dc=example,dc=com', filter: {:and, [and: [equalityMatch: {:AttributeValueAssertion, 'mail', 'jane@example.com'}], and: []]}}}, _from, state) do
    ldap_response = {:ok, {:eldap_search_result, [@jane], []}}
    {:reply, ldap_response, state}
  end
  def handle_call({:search, %{base: 'ou=users,dc=example,dc=com', filter: {:and, [and: [equalityMatch: {:AttributeValueAssertion, 'mail', 'jim@example.com'}], and: []]}}}, _from, state) do
    ldap_response = {:ok, {:eldap_search_result, [@jim], []}}
    {:reply, ldap_response, state}
  end
  def handle_call({:search, %{base: 'ou=users,dc=example,dc=com', filter: {:and, [and: [equalityMatch: {:AttributeValueAssertion, 'mail', 'tom@example.com'}], and: []]}}}, _from, state) do
    ldap_response = {:ok, {:eldap_search_result, [@tom], []}}
    {:reply, ldap_response, state}
  end
  def handle_call({:search, %{base: 'ou=users,dc=example,dc=com', filter: {:and, [and: [equalityMatch: {:AttributeValueAssertion, 'uid', 'jim'}], and: []]}}}, _from, state) do
    ldap_response = {:ok, {:eldap_search_result, [List.first(state)], []}}
    {:reply, ldap_response, state}
  end
  def handle_call({:search, %{base: 'ou=users,dc=example,dc=com', filter: {:and, [and: [equalityMatch: {:AttributeValueAssertion, 'uid', 'tom'}], and: []]}}}, _from, state) do
    ldap_response = {:ok, {:eldap_search_result, [@tom], []}}
    {:reply, ldap_response, state}
  end
  def handle_call({:search, %{base: 'ou=users,dc=example,dc=com', filter: {:and, [and: [equalityMatch: {:AttributeValueAssertion, 'uid', 'jane'}], and: []]}}}, _from, state) do
    ldap_response = {:ok, {:eldap_search_result, [@jane], []}}
    {:reply, ldap_response, state}
  end
  def handle_call({:search, %{base: 'ou=users,dc=example,dc=com', filter: {:and, [and: [equalityMatch: {:AttributeValueAssertion, 'uid', 'jim'}], and: []]}}}, _from, state) do
  ...
  def handle_call({:search, %{base: 'ou=users,dc=example,dc=com', filter: {:and, [and: [equalityMatch: {:AttributeValueAssertion, 'uid', 'matt'}], and: []]}}}, _from, state) do
    ldap_response = {:ok, {:eldap_search_result, [@matt], []}}
    {:reply, ldap_response, state}
  end
  def handle_call({:search, %{base: 'ou=users,dc=example,dc=com', filter: {:and, [and: [], and: [equalityMatch: {:AttributeValueAssertion, 'uid', 'jim'}]]}}}, _from, state) do
    ldap_response = {:ok, {:eldap_search_result, [List.first(state)], []}}
    {:reply, ldap_response, state}
  end
  ...
  def handle_call({:search, search_options}, _from, state) do
    ldap_response = {:ok, {:eldap_search_result, [], []}}
    Logger.error "! ! ! LDAP catch-all invoked ! ! !\nsearch_options: #{inspect search_options}\nstate: #{inspect state}"
    {:reply, ldap_response, state}
  end
  def handle_call({:update, 'uid=matt,ou=users,dc=example,dc=com', modify_operations}, _from, state) do
