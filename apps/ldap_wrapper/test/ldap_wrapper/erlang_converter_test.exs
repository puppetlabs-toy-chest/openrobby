defmodule LdapWrapper.ErlangConverterTest do
  ...
  test "convert_to_erlang of a list of strings gives a list of char lists" do
    assert ErlangConverter.convert_to_erlang(["ldap.example.com", "hello"]) == [
             'ldap.example.com',
             'hello'
           ]

    assert ErlangConverter.convert_to_erlang(["ldap.example.com", "Sören"]) == [
             'ldap.example.com',
             [83, 195, 182, 114, 101, 110]
           ]
  end
  ...
  test "convert_from_erlang of a list of char lists gives a list of strings" do
    assert ErlangConverter.convert_from_erlang(['ldap.example.com', 'hello']) == [
             "ldap.example.com",
             "hello"
           ]

    assert ErlangConverter.convert_from_erlang(['ldap.example.com', [83, 195, 182, 114, 101, 110]]) ==
             ["ldap.example.com", "Sören"]
  end
