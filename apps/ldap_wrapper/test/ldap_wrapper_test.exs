defmodule LdapWrapperTest do
  ...
  test "authenticate returns an error if sent invalid credentials", meta do
    assert LdapWrapper.authenticate(meta[:handle], "user", "mychildrensnames") ==
             {:error, :invalidCredentials}
  end

  test "authenticate with valid credentials returns ok", meta do
    assert LdapWrapper.authenticate(
             meta[:handle],
  ...
  test "search_ldap returns a map of requested attributes for a given user", meta do
    assert LdapWrapper.search_ldap(
             meta[:handle],
             "jane@example.com",
             "ou=users,dc=example,dc=com",
             "mail",
             [:dn, :uid, :objectClass, :mail, :mobile]
           ) ==
             [
               %{
  ...
  test "ldap search by email returns a map of desired attributes to arrays of their values",
       meta do
    assert LdapWrapper.search_by_email(
             meta[:handle],
             "jane@example.com",
             "ou=users,dc=example,dc=com",
             [:dn, :uid, :objectClass, :mail, :mobile]
  ...
  test "change password calls the appropriate ldap function", meta do
    ...
    assert_received {:modify_password, 'uid=tom,ou=users,dc=example,dc=com', 'doggbutt',
                     'cattbutt'}
  end
  ...
  test "If the LDAP response is :ldap_closed, respond with a disconnect/retry error" do
    assert LdapWrapper.parse_search_results({:error, :ldap_closed}) ==
